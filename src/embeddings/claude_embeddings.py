...
class ClaudeEmbeddingsService:
    """Service for generating and managing text embeddings using Claude API."""
    ...
    def client(self):
        ...
        if self._client is None and not self._initialized:
            ...
            try:
                ...
                logger.warning(f"Failed to initialize Anthropic client: {e}. Will use fallback embeddings.")
        return self._client
        ...
        if self.client:
            try:
                response = self.client.messages.create(
                    ...
                    messages=[{
                        "role": "user",
                        "content": f"Generate a semantic representation for: {text[:500]}"
                    }]
                )
                ...
                logger.debug(f"Failed to generate embedding via Claude API: {e}")
                embedding = None

        if embedding is None:
            logger.debug(f"Using fallback embedding for text hash {text_hash}")
