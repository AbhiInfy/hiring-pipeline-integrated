...
3. Use that App Password as the value for `SENDER_PASSWORD` in your `.env` file.

> ❌ Do **not** use your regular Gmail password. It will fail with an authentication error.

### Testing Your Configuration

To verify your email settings without running the full pipeline, use this quick test script:
...
**Custom Matching Threshold** — Lower threshold to get more matches (default is `0.12`):
...
> If you do not see a message in your inbox even when `status=sent`, check your Spam/Promotions folder.
...
| Empty template created | The pipeline creates an empty template — you must add your data |
...
- Start with a lower page count (`--pages 1`) for initial testing.
- Use `heuristic`/`jaccard` mode if adapter mode fails.
- Lower `--min-score` gradually: `0.12` → `0.08` → `0.05`.
